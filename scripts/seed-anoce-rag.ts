...
function uniqueStrings(values: Array<string | number | string[] | number[] | null | undefined>) {
...
function toAnoceRagDocumentRow(document: AnoceRagDocumentMn): AnoceRagDocumentRow {
  const metadata = document.metadata ?? {}
  const brandSlug = metadata.slug ?? null
  const category = metadata.category ?? (document.type === 'brand' ? null : document.type)
...
function normalizeCollection(collection: Collection | LiveCollection, source: 'mock' | 'supabase') {
  if (source === 'supabase') {
...
function collectionRows(
  ...
  const lookSummary = looks
    .map((look) => `Look ${look.number}: ${look.description} Материал: ${look.materials.join(', ') || 'бүртгэлгүй'}. Tags: ${look.tags.join(', ') || 'байхгүй'}.`)
    .join('\n')
  ...
  const rows: AnoceRagDocumentRow[] = [
    {
      ...
      metadata: {
        sourceDataset: source === 'supabase' ? 'supabase-archive' : 'mock-archive',
        source,
...
function demoCollectionRows(collection: DemoArchiveCollection) {
  ...
  const lookSummary = collection.looks
    .map((look) => `Look ${look.number}: ${look.title}. ${look.descriptionMn} Материал: ${look.materialsMn.join(', ')}. Tags: ${look.tags.join(', ')}.`)
    .join('\n')
...
function demoLookRow(collection: DemoArchiveCollection, look: DemoArchiveLook, aliases: string[]): AnoceRagDocumentRow {
  return {
...
function designerRow(designer: Designer | LiveDesigner, source: 'mock' | 'supabase'): AnoceRagDocumentRow {
  const normalized =
    source === 'supabase'
      ? {
  ...
  return {
    ...
    source_confidence: source === 'supabase' ? 'high' : 'medium',
...
function articleRow(article: Article | LiveArticle, source: 'mock' | 'supabase'): AnoceRagDocumentRow | null {
  const normalized =
    source === 'supabase'
      ? {
...
async function fetchLiveArchiveRows(supabase: SupabaseClient) {
  const rows: AnoceRagDocumentRow[] = []

  const [designersRes, collectionsRes, articlesRes] = await Promise.all([
    supabase.from('designers').select('id,slug,name,tier,bio,short_bio,founded'),
    supabase
      .from('collections')
      .select('id,slug,title,designer_name,designer_slug,season,year,description,cover_image,looks(number,description,materials,tags)'),
    supabase
      .from('articles')
      .select('id,slug,title,subtitle,category,author_name,body,status,designer_slug,tags,read_time,published_at')
      .eq('status', 'published'),
  ])

  if (designersRes.error) {
    console.warn(`Could not load designers for RAG seed: ${designersRes.error.message}`)
  } else {
    rows.push(...((designersRes.data ?? []) as LiveDesigner[]).map((designer) => designerRow(designer, 'supabase')))
  }

  if (collectionsRes.error) {
    console.warn(`Could not load collections for RAG seed: ${collectionsRes.error.message}`)
  } else {
    for (const collection of (collectionsRes.data ?? []) as LiveCollection[]) {
      rows.push(...collectionRows(collection, 'supabase', 'high'))
    }
  }

  if (articlesRes.error) {
    console.warn(`Could not load articles for RAG seed: ${articlesRes.error.message}`)
  } else {
    for (const article of (articlesRes.data ?? []) as LiveArticle[]) {
      const row = articleRow(article, 'supabase')
...
async function main() {
  ...
  if (anonKey && serviceRoleKey === anonKey) {
    throw new Error('SUPABASE_SERVICE_ROLE_KEY must be the service role key, not the anon key.')
  }
  ...
  const rows = dedupeRows([...curatedRows, ...localArchiveRows, ...liveArchiveRows])
  ...
  console.log(`Inserted/updated ${data?.length ?? rows.length} Anoce RAG rows.`)
