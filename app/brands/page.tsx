...
export default function BrandsPage() {
  ...
  const highEnd = useMemo(
    () => designers.filter((d) => d.tier === "high-end"),
    [designers],
  );
  const contemporary = useMemo(
    () => designers.filter((d) => d.tier === "contemporary"),
    [designers],
  );
  const emerging = useMemo(
    () => designers.filter((d) => d.tier === "emerging"),
    [designers],
  );
                ...
                <Link key={designer.id} href={`/designers/${designer.slug}`}>
                  ...
                  >
                    <div className="relative aspect-[16/9] overflow-hidden">
                      {designer.cover_image || designer.profile_image ? (
                        <Image
                          src={designer.cover_image || designer.profile_image!}
                          alt={designer.name}
                          fill
              ...
              {contemporary.map((designer, i) => (
                <Link key={designer.id} href={`/designers/${designer.slug}`}>
                  ...
                  >
                    ...
                    <div className="p-6">
                      <h3 className="font-serif text-xl text-[#2A2522] mb-1">
                        {designer.name}
                      </h3>
                ...
                <Link key={designer.id} href={`/designers/${designer.slug}`}>
                  ...
                  >
                    <div className="relative aspect-square overflow-hidden">
                      {designer.profile_image || designer.cover_image ? (
                        <Image
                          src={designer.profile_image || designer.cover_image!}
                          alt={designer.name}
                          fill
                          className="object-cover transition-transform duration-700 group-hover:scale-105"
                        />
                    ...
                    <div className="p-5">
                      <h3 className="font-serif text-lg text-[#2A2522] mb-1">
                        {designer.name}
                      </h3>
                      <span className="font-sans text-xs tracking-[3px] uppercase text-[#9B9590]">
                        Est. {designer.founded}
