...
export const StickyNavbar = () => {
  ...
  }, [isHomePage, pathname, showDropdown, isSearchOpen]);
  ...
  useEffect(() => {
    ...
    const handleGlobalSearchShortcuts = (event: KeyboardEvent) => {
      ...
      const isCommandPalette = event.key.toLowerCase() === 'k' && (event.metaKey || event.ctrlKey);

      if (!isSlash && !isCommandPalette) return;
    ...
    return () => window.removeEventListener('keydown', handleGlobalSearchShortcuts);
  }, []);
