...
import { fetchSearchIndex, RECENT_HISTORY_KEY, groupRankedResults, rankResults } from './searchEngine'
...
export const useGlobalSearch = () => {
  ...
  const activeResults = useMemo(() => {
    return activeCategory === 'all' ? rankedResults : groupedResults[activeCategory]
  ...
  const suggestedFallback = useMemo(
    () => searchIndex.filter((item) => item.category === 'articles' || item.category === 'collections').slice(0, 3),
    [searchIndex]
  )
