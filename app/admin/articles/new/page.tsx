...
import { ChevronLeft, ImagePlus, Upload, Loader2, Check } from 'lucide-react'
...
export default function NewArticlePage() {
  ...
  async function handleSave(publish = false) {
    ...
    try {
      ...
      alert(err.message || 'Failed to save article. Make sure you are logged in.')
    } finally {
      setLoading(false)
    }
  }
  ...
  async function handleImageUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (uploadError) {
        ...
        // For now, use a sample image as placeholder since bucket doesn't exist
    ...
    >
      ...
      <header className="sticky top-0 z-40 bg-white border-b border-[rgba(0,0,0,0.08)] px-0 py-4 flex items-center justify-between w-full">
        <div className="flex items-center gap-4">
          ...
          <h1 className="font-sans text-[11px] tracking-[2px] uppercase text-[#2A2522]">New Article</h1>
        </div>
        <div className="flex items-center gap-4">
          <button 
      ...
      <div className="flex min-h-screen">
        ...
        <div className="w-72 bg-white border-l border-[rgba(0,0,0,0.08)] sticky top-[73px] h-[calc(100vh-73px)] overflow-y-auto px-6 py-8">
          {/* Publish section */}
          <div className="mb-8">
            <h3 className="font-sans text-[10px] tracking-[2px] uppercase text-[#9B9590] mb-4">Publish</h3>
            <div className="space-y-3">
              ...
              >
                {loading ? <Loader2 className="w-4 h-4 animate-spin mx-auto" /> : 'Publish'}
              </button>
            </div>
          </div>
