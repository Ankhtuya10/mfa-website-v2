...
export default function EditCollectionPage() {
  ...
  useEffect(() => {
    async function init() {
      const [collectionData, designersData] = await Promise.all([
        fetchJson<any>(`/api/admin/content/collections/${encodeURIComponent(String(params.id))}`).catch(() => null),
        getDesigners().catch(() => []),
      ]);
      ...
      setCoverPreview(
        d.cover_image ? `${d.cover_image}?v=${Date.now()}` : null,
      );
      setLoadingCollection(false);
    }
  ...
  const finalSlug =
    formData.slug || generatedSlug || `collection-${Date.now()}`;
  ...
  async function handleSave() {
    ...
    try {
      ...
      await postJson(
        `/api/admin/content/collections/${encodeURIComponent(String(params.id))}`,
        payload,
        "PUT",
      );
      ...
      <div className="flex min-h-[calc(100vh-73px)]">
        ...
        <div className="flex-1 overflow-y-auto bg-[#F9F7F4] px-12 py-10">
          ...
          <p className="mb-8 font-sans text-[11px] text-[#9B9590]">
            anoce.mn/archive/{finalSlug || "collection-slug"}
          </p>
