...
const TRENDING_TAGS = ['cashmere', 'fw2025', 'gobi', 'ulaanbaatar', 'wool', 'emerald', 'winter']
...
const highlightMatch = (text: string, query: string) => {
  ...
  return (
    <>
      {text.slice(0, index)}
      <span className="text-white font-medium">{text.slice(index, index + matchedTerm.length)}</span>
...
export default function ExplorePage() {
  ...
  useEffect(() => {
    ...
    setRecentSearches((previous) => {
      const next = [debouncedQuery, ...previous.filter((item) => item !== debouncedQuery)].slice(0, 5)
  ...
  const rankedResults = useMemo(() => {
    ...
    const filteredBySeason = SEARCH_INDEX.filter((item) => {
      ...
      return seasonFilter === 'current'
        ? item.seasonLabel.toLowerCase().includes('ss2025') || item.seasonLabel.toLowerCase().includes('fw2025')
        : !item.seasonLabel.toLowerCase().includes('ss2025') && !item.seasonLabel.toLowerCase().includes('fw2025')
    })
  ...
  const groupedResults = useMemo(() => {
    return {
      ...
      collections: rankedResults.filter((item) => item.category === 'collections'),
    ...
    <div className="h-screen w-full overflow-hidden bg-[#0A0A0A]">
      ...
      <main className="h-full w-full overflow-y-scroll snap-y snap-mandatory scroll-smooth snap-container pt-[72px] md:pt-[88px]">
        <section className="snap-start relative h-screen w-full overflow-hidden bg-[#0A0A0A]">
          ...
          <div className="relative z-10 flex h-full items-center justify-center px-8 py-12 md:px-12 lg:px-16">
            <div className="mx-auto grid w-full max-w-[88rem] grid-cols-1 gap-10 xl:grid-cols-12 xl:items-center">
              <div className="min-w-0 xl:col-span-7">
                ...
                <h1 className="max-w-[11ch] font-serif text-[clamp(3.8rem,8vw,6.8rem)] leading-[0.92] tracking-[-0.03em] text-white [text-wrap:balance]">
                ...
                <p className="mt-6 max-w-xl font-sans text-lg leading-relaxed text-white/64 md:text-xl">
                  Search collections, articles, designers, brands, materials, colors, and seasonal terms with live grouped results.
                </p>
              ...
              <div className="min-w-0 xl:col-span-5">
                <AnimatePresence mode="wait">
                  ...
                  >
                    ...
                    <div className="min-w-0 px-1 pt-5">
                      <span className="font-sans text-[10px] tracking-[0.28em] uppercase text-[#B7AEA9]">Editor&apos;s Choice</span>
                      <h2 className="mt-3 max-w-[16ch] font-serif text-[clamp(2.1rem,3.1vw,3rem)] leading-[1.02] text-white [overflow-wrap:anywhere] [text-wrap:balance]">
                        {spotlightItem?.title}
                      </h2>
                      <p className="mt-3 max-w-[54ch] font-sans text-sm leading-relaxed text-white/58">{spotlightItem?.subtitle}</p>
                      <Link href={spotlightItem?.href || '/archive'} className="mt-6 inline-flex max-w-full items-center gap-2 rounded-full border border-white/[0.12] bg-white/[0.05] px-5 py-2.5 transition-all hover:border-white/[0.24] hover:bg-white/[0.08]">
                        <span className="font-sans text-[10px] tracking-[0.22em] uppercase whitespace-nowrap text-white/78">Open Feature</span>
                ...
                <div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2">
                  <div className="min-w-0 rounded-[30px] border border-white/[0.12] bg-[linear-gradient(180deg,rgba(255,255,255,0.09),rgba(255,255,255,0.03))] p-5 shadow-[0_20px_60px_rgba(0,0,0,0.18)] backdrop-blur-xl">
                    <p className="mb-4 font-sans text-[10px] tracking-[0.28em] uppercase text-white/32">Recent History</p>
                    <div className="space-y-3">
                      {recentSearches.length > 0 ? recentSearches.map((item) => (
                        ...
                        >
                          ...
                          <span className="truncate font-sans text-sm">{item}</span>
                        </button>
                      )) : (
                        <p className="font-sans text-sm leading-relaxed text-white/42">Your last searches will appear here after you start exploring.</p>
                      )}
                    </div>
                  </div>

                  <div className="min-w-0 rounded-[30px] border border-white/[0.12] bg-[linear-gradient(180deg,rgba(255,255,255,0.09),rgba(255,255,255,0.03))] p-5 shadow-[0_20px_60px_rgba(0,0,0,0.18)] backdrop-blur-xl">
                    <p className="mb-4 font-sans text-[10px] tracking-[0.28em] uppercase text-white/32">Search Scope</p>
                    <div className="space-y-3 font-sans text-sm leading-relaxed text-white/60">
                      <p>Titles, body text, tags, materials, colors, seasons, and brand metadata.</p>
                      <p>Typo-tolerant search and term mapping for inputs like “winter”, “fw”, or “cashmer”.</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="mb-8 flex flex-wrap items-center justify-between gap-4 md:mb-10">
              <div>
                ...
                <h2 className="font-serif text-3xl leading-[1.05] text-white md:text-4xl lg:text-5xl">
                  {showLiveResults ? 'Top Matches by Category' : 'Start with visual discovery'}
                </h2>
              </div>
              <div className="flex flex-wrap gap-2">
                {(['all', 'articles', 'collections', 'designers', 'brands'] as const).map((category) => (
                  <button
                    key={category}
                      ...
                      return (
                        <section key={category}>
                          ...
                          <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
                            {items.map((item) => (
                              <ResultCard key={item.id} item={item} query={deferredQuery} />
                            ))}
                          </div>
                        </section>
                      )
        ...
        <section className="snap-start relative h-screen w-full overflow-hidden border-t border-white/[0.06] bg-[#0F0D0B] px-8 pb-10 pt-[108px] md:px-12 md:pb-12 md:pt-[132px] lg:px-16">
          <div className="mx-auto flex h-full min-h-0 w-full max-w-[88rem] flex-col">
            <div className="mb-8 text-center md:mb-10">
              ...
              <h2 className="font-serif text-3xl leading-[1.05] text-white md:text-4xl lg:text-5xl">
                {showLiveResults ? `${totalResults} ranked matches` : 'Submit a term to unlock the full result grid'}
              </h2>
            </div>
                ...
                <div className="flex h-full items-center justify-center text-center">
                  <div className="max-w-2xl">
                    <p className="font-sans text-lg leading-relaxed text-white/48">
                      Search will expand into a scored result grid here, with matches ranked higher for titles and rich metadata.
                    </p>
                  </div>
                </div>
              ) : (
